...
#Define some formula
Y<-  1 + 1*X[, 1] + sqrt(2)*X[, 2] + sqrt(3)*X[, 3] + gamma + eta + epsilon # + eta
  ...
  p <- ggplot(df_combined, aes(x = x, y = y, color = Effect)) +
    ...
    scale_color_manual(values = rainbow(6), 
                       labels = c(expression(beta[0]^2 ~ 'Posterior mean=0.0000'),
                                expression(sigma[alpha]^2 ~ 'Posterior mean=0.0557'),
                                expression(paste(sigma[epsilon]^2) ~ 'Posterior mean=0.0539'),
                                expression(beta[1]^2 ~ 'Posterior mean=0.2649'),
                                expression(beta[2]^2 ~ 'Posterior mean=0.3011'),
                                expression(beta[3]^2 ~ 'Posterior mean=0.3264'))) 
...
# output_folder_plots <- "/Users/augustarnstad/Library/CloudStorage/OneDrive-NTNU/Semester 9/Prosjekt/Latex/Thesis template NTNU/Figures/Posterior Marginal"
# 
